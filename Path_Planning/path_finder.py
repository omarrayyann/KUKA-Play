...
def find_nearest_free_point(self,point, grid_points, grid_labels, min_distance=0.4):
...
def optimize_waypoints(self, waypoints, grid, grid_labels):
    ...
    Utils.print_debug(f"Reduced number of waypoints from {len(optimized_path)} to {len(waypoints)}",self.args.debug,module_id)
