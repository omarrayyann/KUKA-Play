import sys
sys.path.append("/Users/omarrayyann/Documents/Robotics/")
from manipulator import Manipulator, Utils
import numpy as np
import time
import scipy
...
def get_error(joint_angles, desired_position, initial_joint_angles, manip):
    current_position = manip.forward_kinematics(joint_angles.reshape(len(joint_angles),-1).tolist())[0:3,3]
...
def inverse_kinematics(manip, desired_position, current_q):
    q0 = np.zeros(7)
    res = scipy.optimize.minimize(get_error, q0, args = (desired_position,current_q,manip))
