...
def move_grasp(self, position, rotation=None, eef_type=1 , lift_arm_first=False, move_first=False):
    ...
    if eef_type == 3:
        ...
        res_all_base = scipy.optimize.minimize(get_error, q0, args=(self, position, rotation), bounds=bounds_freeze_all_base, tol=0.0001, options={"maxiter": 10000})
        results.append((res_all_base, 0))
        if res_all_base.fun >= 0.0002:
            self.mjdata_fake = copy.deepcopy(self.mjdata)
            res_base_position = scipy.optimize.minimize(get_error, q0, args=(self, position, rotation), bounds=bounds_freeze_base_position, tol=0.0001, options={"maxiter": 10000})
            results.append((res_base_position, 1))
            if res_base_position.fun >= 0.0002:
                self.mjdata_fake = copy.deepcopy(self.mjdata)
                res_forward_backward = scipy.optimize.minimize(get_error_constrained, q0, args=(position, rotation), bounds=bounds_allow_forward_backward, tol=0.0001, options={"maxiter": 10000})
        ...
        if eef_type == 2:
            self.mjdata_fake = copy.deepcopy(self.mjdata)
            res = scipy.optimize.minimize(get_error_constrained, q0, args=(position, rotation), bounds=bounds_allow_forward_backward, tol=0.0001, options={"maxiter": 10000})
        else:
            self.mjdata_fake = copy.deepcopy(self.mjdata)
            res = scipy.optimize.minimize(get_error, q0, args=(self, position, rotation), bounds=bounds_freeze_all_base if eef_type == 0 else bounds_freeze_base_position, tol=0.0001, options={"maxiter": 10000})
