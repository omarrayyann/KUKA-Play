...
def pick(self, object, visualize_path=False):
    ...
    waypoints = Path_Planning.find_path(self,self.pcd_points,self.pcd_labels,starting_position,ending_position,0.0,visualize_path)   
    ...
    while not successfully_picked:
        ...
        if not self.args.disable_any_grasp:
            ...
            grasp_poses, _, grasp_widths, _, langsam_indicies, trajectory_indicies = Grasping.get_grasps(rgb,depth,object.name,self.args.trajectory_length,self.args.trajectory_radius,self.args.grasp_server_host,self.args.grasp_server_port)
        ...
        Utils.set_body_pose(self.mjmodel,"grasping_head_0",optimal_grasp_pose[0:3,3],optimal_grasp_pose[0:3,0:3],self.args.debug)
        trajectory_positions, extra_pos = Utils.get_trajectory_steps(optimal_grasp_pose,self.args.trajectory_steps,self.args.trajectory_length)
        for i, trajectory_position in enumerate(trajectory_positions):
            ...
            Manipulation.move_grasp(self,trajectory_position,optimal_grasp_pose[0:3,0:3],3,lift_arm_first=i==0,move_first=i==0)
            Utils.sleep(0.1)
        Utils.sleep(1)
        ...
        if not successfully_picked:
            Manipulation.ungrasp(self,optimal_grasp_width)
            Utils.sleep(0.2)
            Utils.set_body_pose(self.mjmodel,"grasping_head",trajectory_position,optimal_grasp_pose[0:3,0:3])
            Manipulation.move_grasp(self,extra_pos,optimal_grasp_pose[0:3,0:3],3,False)
            Utils.sleep(0.2)
            successfully_picked = Manipulation.grasp(self)
