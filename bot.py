...
class UntidyBotSimulator():
    ...
    def start(self) -> None:
        threading.Thread(target=self.__run_mac).start()
        time.sleep(0.5)
        self.home()
        time.sleep(0.5)

        if self.args.instruction:
            threading.Thread(target=self.instruct, args=(self.args.instruction,)).start()


    def instruct(self, instruction):

        # gpt_output = call_gpt_with_json(self.objects_json_path,self.relations_json_path,instruction,self.gpt_model)
        # actions = GPT.parse_output(gpt_output)

        actions = [Classes.Action("pick",Classes.Object("cup",[1.39,-1.01,0.82])),Classes.Action("place",Classes.Object("plate bin",[-1.52,1.02,0.78]))]
    ...
    while True:
        ...
        if not robot_sim.args.no_camera_visualization:
            ...
            if robot_sim.args.rerun:
                ...
                rr.log("RealSense D435i", rr.Image(cam_d435i_rgb))
            else:
